...
use crate::{Decision, Intent, Observation, Pos, Role, Strategy, Team, CARGO_CAPACITY};
...
impl BaselineBot {
    pub const fn new(strategy: Strategy) -> Self { Self { strategy } }
}
...
impl Bot for BaselineBot {
    fn decide(&mut self, view: &Observation) -> Decision {
        ...
        if drone.position == view.base && drone.cargo > 0 {
            return Decision::new(Intent::Deposit, Role::Courier, Some(view.base), "Unloading at home dock");
        }
        if drone.cargo == CARGO_CAPACITY {
            return Decision::new(move_toward(view, view.base), Role::Courier, Some(view.base), "Cargo full — returning home");
        }
        if view.known_crystals.get(&drone.position).copied().unwrap_or(0) > 0 {
            return Decision::new(Intent::Harvest, Role::Harvester, Some(drone.position), "Extracting visible crystal");
        }

        let mut known: Vec<Pos> = view.known_crystals.iter()
            .filter_map(|(p, amount)| (*amount > 0).then_some(*p)).collect();
        known.sort_by_key(|p| (p.x, p.y));
        let coverage_limit = (view.width * view.height) as usize * SCOUT_COVERAGE_LIMIT_PERCENT / 100;
        let needs_more_intel = known.len() < SCOUT_SOURCE_GOAL && view.explored.len() < coverage_limit;
        let should_scout = match self.strategy {
            Strategy::Autonomous => false,
            Strategy::DedicatedScout => drone.id == 0 && view.explored.len() < (view.width * view.height) as usize,
            Strategy::HybridScout => drone.id == 0 && needs_more_intel,
        };
        if should_scout {
            let target = drone.target.filter(|target| !view.explored.contains(target))
        ...
        if !known.is_empty() {
            ...
            if drone.blocked_turns >= 4 {
                if let Some(fallback) = frontier_target(view, drone.position, drone.id + drone.blocked_turns as usize) {
                    return Decision::new(move_toward_for_drone(view, fallback), Role::Scout, Some(fallback), "Route stalled — scouting a fresh approach");
                }
            }
            ...
            if !matches!(action, Intent::Wait) || drone.position == target {
                let reason = if self.strategy == Strategy::Autonomous { "Heading to nearest known resource" } else { "Assigned to a team resource" };
            ...
            if let Some(fallback) = frontier_target(view, drone.position, drone.id) {
                return Decision::new(move_toward_for_drone(view, fallback), Role::Scout, Some(fallback), "Resource route blocked — rerouting");
            }
            return Decision::new(Intent::Wait, Role::Courier, Some(target), "Waiting for route to resource");
        }

        if let Some(target) = frontier_target(view, drone.position, drone.id) {
            Decision::new(move_toward_for_drone(view, target), Role::Scout, Some(target), "Searching for energy signatures")
        } else if drone.cargo > 0 {
            Decision::new(move_toward(view, view.base), Role::Courier, Some(view.base), "No resources remain — banking cargo")
        } else {
            let parking = parking_spot(view, drone.team, drone.id);
            if drone.position != parking {
                Decision::new(move_toward_for_drone(view, parking), Role::Courier, Some(parking), "No task — clearing the shipping lane")
            } else {
                Decision::new(Intent::Wait, Role::Courier, Some(parking), "Parked clear of the shipping lane")
            }
        }
    }
}
...
fn move_toward(view: &Observation, target: Pos) -> Intent {
    if view.me.position == target { return Intent::Wait; }
    ...
    if !previous.contains_key(&target) { return Intent::Wait; }
    let mut step = target;
    while previous[&step] != view.me.position { step = previous[&step]; }
    Intent::Move(step)
}
...
fn route_distance(view: &Observation, from: Pos, target: Pos) -> Option<i32> {
    ...
    while let Some((current, distance)) = queue.pop_front() {
        ...
        for next in current.neighbors() {
            if passable(view, next) && visited.insert(next) { queue.push_back((next, distance + 1)); }
        }
    }
    None
}
...
fn move_toward_for_drone(view: &Observation, target: Pos) -> Intent {
    ...
    let mut alternatives: Vec<(i32, Pos)> = drone.position.neighbors().into_iter()
        ...
        .filter_map(|candidate| route_distance(view, candidate, target).map(|distance| (distance, candidate)))
        .collect();
    alternatives.sort_by_key(|(distance, candidate)| (*distance, candidate.x, candidate.y));
    alternatives.first().map_or(direct, |(_, candidate)| Intent::Move(*candidate))
}

fn frontier_target(view: &Observation, from: Pos, slot: usize) -> Option<Pos> {
    let mut candidates: Vec<Pos> = (0..view.width).flat_map(|x| (0..view.height).map(move |y| Pos::new(x, y)))
    ...
    candidates.into_iter().find(|candidate| !matches!(move_toward(view, *candidate), Intent::Wait))
}
