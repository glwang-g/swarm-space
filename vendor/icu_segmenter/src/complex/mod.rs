...
impl<'data> ComplexPayloadsBorrowed<'data> {
    fn select(&self, language: Language) -> Option<DictOrLstmBorrowed<'data>> {
        ...
        match language {
            ...
            // UAX #14 already supplies CJK line-break opportunities. The
            // dictionary is optional for this caller, so a missing one is a
            // normal fallback, not an error to emit once per text layout.
