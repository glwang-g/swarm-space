...
use bevy::window::{MonitorSelection, WindowMode, WindowResized, WindowResolution};
use std::sync::{
    Arc,
...
use swarm_core::*;
use swarm_runner::{
    MatchCommand, RENDER_CARGO_CAPACITY, RenderSnapshot, RunnerHandle, RunnerUpdate, spawn_runner,
};
mod bots;
...
fn make_runner(seed: u64, setup: &ArenaSetup, bot_mode: &Arc<AtomicBool>) -> RunnerHandle {
    ...
    spawn_runner(seed, setup.scenario(), move |team, _id| {
        if bot_mode.load(Ordering::Relaxed) && team == Team::Azure {
            ...
            Box::new(BaselineBot::new(if team == Team::Azure {
                Strategy::Autonomous
            } else {
                Strategy::HybridScout
            }))
        }
    })
}
...
impl ArenaSetup {
    fn scenario(&self) -> Scenario {
        let mut scenario = Scenario::scaled(
            self.drones_per_team,
            [Strategy::Autonomous, Strategy::HybridScout],
        );
...
struct WorldVisual;
#[derive(Component)]
struct DroneVisual(Team, usize);
#[derive(Component)]
struct DroneLabel(Team, usize);
#[derive(Component)]
struct CrystalVisual(Pos);
#[derive(Component)]
struct TargetVisual(Team, usize);
#[derive(Component)]
struct FogVisual(Pos);
#[derive(Component)]
struct ScoreText;
#[derive(Component)]
struct StatusText;
...
struct IntroOverlay;
#[derive(Component)]
struct ArenaSetupText;
#[derive(Component)]
struct CustomBotText;
#[derive(Component)]
...
fn setup(mut commands: Commands, assets: Res<AssetServer>, windows: Query<&Window>) {
    ...
    let snapshot = runner
        ...
        .and_then(|update| match update {
            RunnerUpdate::Snapshot(snapshot) => Some(snapshot),
            _ => None,
        })
        .expect("runner initial snapshot");
    ...
    commands.spawn((
        ...
        Camera {
            ...
            viewport: Some(map_viewport(window)),
            ..default()
        },
    ));
...
fn map_viewport(window: &Window) -> Viewport {
    ...
    Viewport {
        ...
        physical_size: UVec2::new(
            window.resolution.physical_width().saturating_sub(panel),
...
fn minimap_content_rect(window: &Window, layout: BoardLayout) -> (Vec2, Vec2, f32) {
    ...
    (
        frame_top_left + Vec2::splat(padding) + (Vec2::splat(MINIMAP_SIZE) - map_size) * 0.5,
        map_size,
        scale,
    )
}
...
fn spawn_world(commands: &mut Commands, sim: &RenderSnapshot, layout: BoardLayout) {
    for x in 0..sim.scenario.width {
        for y in 0..sim.scenario.height {
            let p = Pos::new(x, y);
            let color = if sim.walls.contains(&p) {
                WALL
            } else {
                SKY_TILE
            };
            let size = if sim.walls.contains(&p) {
                layout.cell - 3.0
            } else {
                layout.cell - 1.0
            };
            commands.spawn((
                ...
                Transform::from_translation(grid_translation(layout, p, 0.0)),
            ));
        }
    }
    ...
    for x in 0..sim.scenario.width {
        let label = char::from_u32(u32::from(b'A') + x as u32)
            .unwrap_or('?')
            .to_string();
    ...
    for team in Team::ALL {
        ...
        commands.spawn((
            WorldVisual,
            Sprite::from_color(color.with_alpha(0.32), Vec2::splat(layout.cell * 1.65)),
            Transform::from_translation(grid_translation(layout, sim.bases[team.index()], 1.0)),
        ));
        commands.spawn((
            WorldVisual,
            Sprite::from_color(color, Vec2::new(layout.cell * 0.72, layout.cell * 0.18)),
            Transform::from_translation(grid_translation(layout, sim.bases[team.index()], 2.0)),
        ));
    }
    for crystal in &sim.crystals {
        commands.spawn((
            WorldVisual,
            CrystalVisual(crystal.position),
            Sprite::from_color(CRYSTAL, Vec2::splat(layout.cell * 0.42)),
            Transform {
                translation: grid_translation(layout, crystal.position, 3.0),
    ...
    for drone in &sim.drones {
        let color = if drone.team == Team::Azure {
            AZURE
        } else {
            AMBER
        };
        commands.spawn((
            WorldVisual,
            TargetVisual(drone.team, drone.id),
            Sprite::from_color(color.with_alpha(0.18), Vec2::splat(layout.cell * 0.72)),
            Transform::from_translation(grid_translation(layout, drone.position, 2.5)),
            Visibility::Hidden,
        ));
        commands.spawn((
            WorldVisual,
            DroneVisual(drone.team, drone.id),
            Sprite::from_color(color, Vec2::new(layout.cell * 0.66, layout.cell * 0.52)),
            Transform::from_translation(grid_translation(layout, drone.position, 5.0)),
        ));
        let label = format!(
            "{}{}",
        ...
        commands.spawn((
            ...
            Transform::from_translation(
                grid_translation(layout, drone.position, 6.0) + Vec3::new(0.0, -1.0, 0.0),
            ),
        ));
    }
    for x in 0..sim.scenario.width {
        for y in 0..sim.scenario.height {
            ...
            commands.spawn((
                ...
                FogVisual(p),
                Sprite::from_color(
                    Color::srgba(0.005, 0.012, 0.035, 0.94),
                    Vec2::splat(layout.cell),
                ),
                Transform::from_translation(grid_translation(layout, p, 10.0)),
                Visibility::Hidden,
            ));
        }
    }
...
fn spawn_minimap(commands: &mut Commands, sim: &RenderSnapshot, layout: BoardLayout) {
    let scale = MINIMAP_SIZE / layout.size.x.max(layout.size.y);
    let map_size = layout.size * scale;
    let padding = 9.0;
    let outer = MINIMAP_SIZE + padding * 2.0;
        ...
        .with_children(|frame| {
            ...
            frame
                .spawn((
                    ...
                    Node {
                        position_type: PositionType::Absolute,
                        left: Val::Px(padding + offset.x),
                        top: Val::Px(padding + offset.y),
                        width: Val::Px(map_size.x),
                        height: Val::Px(map_size.y),
                        ..default()
                    },
                ...
                .with_children(|map| {
                    for x in 0..sim.scenario.width {
                        for y in 0..sim.scenario.height {
                            let pos = Pos::new(x, y);
                            let color = if sim.walls.contains(&pos) {
                                WALL
                            } else if sim
                                .crystals
                                .iter()
                                .any(|crystal| crystal.position == pos && crystal.amount > 0)
                            {
                                CRYSTAL
                            } else {
                                SKY_TILE
                            ...
                            map.spawn((
                                ...
                                Node {
                                    ...
                                    top: Val::Px(
                                        (sim.scenario.height - 1 - y) as f32 * layout.cell * scale,
                                    ),
                                    width: Val::Px((layout.cell * scale).ceil()),
                                    height: Val::Px((layout.cell * scale).ceil()),
                                    ..default()
                                },
                    ...
                    for team in Team::ALL {
                        ...
                        map.spawn((
                            ...
                            Node {
                                ...
                                top: Val::Px(
                                    (sim.scenario.height - 1 - base.y) as f32 * layout.cell * scale,
                                ),
                                width: Val::Px((layout.cell * scale).max(3.0)),
                                height: Val::Px((layout.cell * scale).max(3.0)),
                                ..default()
                            },
                            BackgroundColor(color),
                        ));
...
fn text_style(size: f32, color: Color) -> (TextFont, TextColor) {
    (TextFont::from_font_size(size), TextColor(color))
}
...
fn spawn_ui(commands: &mut Commands) {
    ...
    commands
        .spawn((
            ...
            BackgroundColor(Color::srgba(0.015, 0.025, 0.055, 0.76)),
            Visibility::Hidden,
        ))
        .with_children(|p| {
            p.spawn((
                EndText,
                Text::new(""),
                text_style(38.0, Color::WHITE).0,
                text_style(38.0, Color::WHITE).1,
...
fn controls(
    ...
    if keys.just_pressed(KeyCode::BracketLeft) {
        setup.drones_per_team = setup.drones_per_team.saturating_sub(1).max(2);
    }
    if keys.just_pressed(KeyCode::BracketRight) {
        setup.drones_per_team = (setup.drones_per_team + 1).min(16);
    }
    if keys.just_pressed(KeyCode::Comma) {
        setup.crystal_sites = setup.crystal_sites.saturating_sub(2).max(5);
    }
    if keys.just_pressed(KeyCode::Period) {
        setup.crystal_sites = (setup.crystal_sites + 2).min(81);
    }
    if keys.just_pressed(KeyCode::Semicolon) {
        setup.wall_chance_percent = setup.wall_chance_percent.saturating_sub(2);
    }
    if keys.just_pressed(KeyCode::Quote) {
        setup.wall_chance_percent = (setup.wall_chance_percent + 2).min(35);
    }
    if keys.just_pressed(KeyCode::Digit1) {
        state.speed = 1;
        let _ = state.runner.send(MatchCommand::SetSpeed(1));
    }
    if keys.just_pressed(KeyCode::Digit2) {
        state.speed = 4;
        let _ = state.runner.send(MatchCommand::SetSpeed(4));
    }
    if keys.just_pressed(KeyCode::Digit3) {
        state.speed = 16;
        let _ = state.runner.send(MatchCommand::SetSpeed(16));
    }
    if keys.just_pressed(KeyCode::KeyM) {
        setup.my_bot = !setup.my_bot;
        state.bot_mode.store(setup.my_bot, Ordering::Relaxed);
        state.paused = true;
        let _ = state.runner.send(MatchCommand::SetRunning(false));
    }
    if keys.just_pressed(KeyCode::KeyN) && (state.paused || state.guided) {
        let _ = state.runner.send(MatchCommand::Step);
        state.paused = true;
    }
    ...
    if restart || regenerate {
        ...
        state.bot_mode.store(setup.my_bot, Ordering::Relaxed);
        let _ = state.runner.send(MatchCommand::Restart {
            seed: state.seed,
...
fn launch_benchmark(lab: &mut ExperimentLab) {
    if lab.receiver.is_some() {
        lab.benchmark_status =
            "Benchmark is already running — please wait for this sample set.".into();
        return;
    }
    ...
    lab.benchmark_status = format!(
        "Running: 2–{max_drones} drones/team, {samples} seeded maps, both side assignments…"
    );
...
fn update_lab(
    ...
    mut overlays: Query<&mut Visibility, With<LabOverlay>>,
    mut text: Query<&mut Text, With<LabText>>,
) {
    ...
    if !updates.is_empty() {
        ...
        for update in updates {
            match update {
                BenchmarkUpdate::Progress { completed, total } => {
                    lab.benchmark_progress = Some((completed, total))
                }
    ...
    if let Ok(mut value) = text.single_mut() {
        let mut lines = vec![
            format!(
                "Selected preview: {} drones/team · Enter to play it",
                lab.selected_drones
            ),
            format!(
                "Benchmark range: 2–{} drones/team · {} seeded maps each · B to run",
                lab.max_drones, lab.samples
            ),
        ...
        if let Some((completed, total)) = lab.benchmark_progress {
            ...
            lines.push(format!(
                "[{}{}] {percent:>3}%  {completed}/{total} matches",
                "#".repeat(filled),
                ".".repeat(20 - filled)
            ));
        }
        for row in &lab.rows {
            let dedicated = format!(
                "{:+.1} / {:>4.0}%",
                row.dedicated_delta, row.dedicated_win_rate
            );
            let hybrid = format!("{:+.1} / {:>4.0}%", row.hybrid_delta, row.hybrid_win_rate);
            let best = if row.dedicated_delta > 0.0 && row.dedicated_delta >= row.hybrid_delta {
                "SCOUT"
            } else if row.hybrid_delta > 0.0 {
                "HYBRID"
            } else {
                "AUTO"
            };
            lines.push(format!(
                " {:>5} | {:>21} | {:>18} | {best}",
                row.drones_per_team, dedicated, hybrid
            ));
        }
...
fn update_arena_setup(setup: Res<ArenaSetup>, mut text: Query<&mut Text, With<ArenaSetupText>>) {
    ...
    if let Ok(mut text) = text.single_mut() {
        **text = format!(
            "公平镜像：双方数值相同\n\n[  {}  ]  每队无人机     [ / ]\n[  {}  ]  晶体点         , / .\n[ {}% ]  障碍密度       ; / '\n\nM · 蓝队使用 MyBot：{}\nENTER · APPLY & RESTART",
            setup.drones_per_team,
            setup.crystal_sites,
            setup.wall_chance_percent,
            if setup.my_bot { "ON" } else { "OFF" },
        );
    }
}
...
fn map_camera_controls(
    ...
    let pointer_in_minimap = window.cursor_position().is_some_and(|cursor| {
        cursor.x >= minimap_origin.x
            && cursor.x <= minimap_origin.x + minimap_size.x
            && cursor.y >= minimap_origin.y
            && cursor.y <= minimap_origin.y + minimap_size.y
    });
    let pointer_in_map = window
        .cursor_position()
        .is_some_and(|cursor| cursor.x < playfield_width)
        && !pointer_in_minimap;
    let Ok((mut transform, mut camera_projection)) = cameras.single_mut() else {
        return;
    };
    let Projection::Orthographic(projection) = &mut *camera_projection else {
        return;
    };
    ...
    if pointer_in_map {
        for event in wheel.read() {
            let amount = match event.unit {
                ...
                MouseScrollUnit::Pixel => event.y / 38.0,
            };
            if amount > 0.0 {
...
fn minimap_controls(
    ...
    let (Ok(window), Ok((mut camera, projection))) = (windows.single(), cameras.single_mut())
    else {
        return;
    };
...
fn sync_minimap_viewport(
    ...
    let (Ok(window), Ok((camera, projection)), Ok((mini, mut node))) =
        (windows.single(), cameras.single(), viewport.single_mut())
    else {
        return;
    };
    let Projection::Orthographic(projection) = projection else {
        ...
        match update {
            RunnerUpdate::Snapshot(snapshot) => {
                ...
                if rebuild {
                    ...
                    *layout =
                        BoardLayout::for_scenario(state.snapshot.scenario, window_size(&windows));
                    spawn_world(&mut commands, &state.snapshot, *layout);
                }
            }
            RunnerUpdate::Event(event) => match event {
                swarm_core::MatchEvent::TurnAdvanced { .. } => {}
...
fn sync_visuals(
    ...
    for (marker, mut transform, mut sprite, mut visibility) in &mut visuals.p0() {
        if let Some(drone) = state
            .snapshot
            .drones
            .iter()
            .find(|d| d.team == marker.0 && d.id == marker.1)
        {
            transform.translation = grid_translation(*layout, drone.position, 5.0);
            let fullness = drone.cargo as f32 / RENDER_CARGO_CAPACITY as f32;
            sprite.color = if drone.team == Team::Azure {
                AZURE
            } else {
                AMBER
            }
            .mix(&Color::WHITE, fullness * 0.35);
            *visibility = match viewed_team {
                Some(team)
                    if drone.team != team
                        && !state.snapshot.currently_visible(team, drone.position) =>
                {
                    Visibility::Hidden
                }
    ...
    for (marker, mut transform, mut visibility) in &mut visuals.p2() {
            ...
            .and_then(|d| d.target)
        {
            ...
            *visibility = if viewed_team.map_or(true, |team| team == marker.0) {
                Visibility::Visible
            } else {
                Visibility::Hidden
            };
    ...
    for (marker, mut visibility) in &mut visuals.p3() {
        let amount = match viewed_team {
            Some(team) => state.snapshot.memories[team.index()]
                .known_crystals
                .get(&marker.0)
                .copied()
                .unwrap_or(0),
            None => state
                ...
                .crystals
                .iter()
                .find(|c| c.position == marker.0)
                .map_or(0, |c| c.amount),
        };
        *visibility = if amount > 0 {
            Visibility::Visible
        } else {
            Visibility::Hidden
        };
    ...
    for (marker, mut sprite, mut visibility) in &mut visuals.p4() {
        let azure_knows = state.snapshot.memories[Team::Azure.index()]
            .explored
            .contains(&marker.0);
        let amber_knows = state.snapshot.memories[Team::Amber.index()]
            .explored
            .contains(&marker.0);
        match viewed_team {
            Some(team) => {
                *visibility = if state.snapshot.memories[team.index()]
                    .explored
                    .contains(&marker.0)
                {
                    Visibility::Hidden
...
fn update_ui(
    ...
    if let Ok(mut text) = texts.p0().single_mut() {
        **text = format!("{}  :  {}", sim.scores[0], sim.scores[1]);
    }
    if let Ok(mut text) = texts.p1().single_mut() {
        let remaining: u32 = sim
            .crystals
            .iter()
            .map(|crystal| crystal.amount as u32)
            .sum();
        let view = match state.view_team {
            None => "OMNISCIENT",
            Some(Team::Azure) => "AZURE MEMORY",
            Some(Team::Amber) => "AMBER MEMORY",
        };
        **text = format!(
            "AZURE  {}       AMBER  Explorer Bot\nTurn {:03} / {}   {}   Speed {}×\nView: {}   Crystals remaining: {}",
            if setup.my_bot { "MyBot" } else { "Greedy Bot" },
            sim.turn,
            sim.scenario.max_turns,
            if state.intro {
                "READY"
    ...
    if let Ok(mut node) = fill.single_mut() {
        node.width = Val::Percent(sim.turn as f32 / sim.scenario.max_turns as f32 * 100.0);
    }
    let visible = sim.finished;
    if let Ok(mut value) = overlays.p0().single_mut() {
        *value = if visible {
            Visibility::Visible
        } else {
            Visibility::Hidden
    ...
    if let Ok(mut value) = overlays.p1().single_mut() {
        *value = if state.intro {
            Visibility::Visible
        } else {
            Visibility::Hidden
        };
    ...
    if visible {
        if let Ok(mut text) = texts.p4().single_mut() {
            let winner = if sim.scores[0] > sim.scores[1] {
                "AZURE WINS"
            } else if sim.scores[1] > sim.scores[0] {
                "AMBER WINS"
            } else {
                "DRAW"
            };
            **text = format!(
                ...
                winner, sim.scores[0], sim.scores[1]
            );
        }
    }
}
